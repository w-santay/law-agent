...
def init_tracer(
    service_name: str = "law-multi-agent",
...
def get_tracer():
    ...
    if _tracer is None:
        if _HAS_OTEL:
            _tracer = trace.get_tracer("law-multi-agent")
        else:
            return None
    return _tracer
...
def set_global_metrics(metrics: AgentMetrics):
    """设置全局指标收集器"""
    global _global_metrics
    _global_metrics = metrics


def get_global_metrics() -> AgentMetrics | None:
    """获取全局指标收集器"""
...
class AgentMetrics:
    ...
    def record_call(self, agent_name: str, duration_ms: float, success: bool):
        print(f"Agent {agent_name} called, duration={duration_ms:.2f}ms, success={success}")
